...
class CFG(object):
    ...
    def assert_sinks(self):
        instructions = {}
        assert_bbs = [
            bb for bb in self.bbs if len(bb.ins) == 1 and hex(bb.ins[0].op) == "0xfe"
        ]
    ...
    def call_sinks(self):
        ...
        call_insn = [
            ...
            for bb in self.bbs
            for ins in bb.ins
            if ins.name in set(["CALL"]) and 0 in bb.ancestors | {bb.start}
        ]
        for call_ins in call_insn:
            ...
            if len(call_succ) == 1 and [ins.name for ins in call_bb_ins[-3:]] == [
                "ISZERO",
                "PUSH2",
                "JUMPI",
            ]:  # propagate throw
                continue
            if len(call_succ) == 2 and [
                ins.name
                for ins in call_bb_ins[call_bb_ins.index(call_ins) + 1 :]
                if ins.name in set(["ADD", "AND", "ISZERO", "JUMPI"])
            ] == ["ADD", "AND", "ISZERO", "JUMPI"]:  # propagate throw
                continue

            call_ins_index = call_bb_ins.index(call_ins)
            if [
                ins.name
                for ins in call_bb_ins[call_ins_index + 1 : call_ins_index + 22]
                if ins.name in set(["ADD", "MSTORE", "MLOAD", "SUB", "SHA3"])
            ] == ["ADD", "MSTORE", "ADD", "MLOAD", "SUB", "SHA3"]:
                continue
            if [
                ins.name
                for ins in call_bb_ins[call_ins_index + 1 : call_ins_index + 17]
                if ins.name in set(["ADD", "MLOAD", "SUB", "SHA3"])
            ] == ["ADD", "MLOAD", "SUB", "SHA3"]:
                continue
            if [
                ins.name
                for ins in call_bb_ins[call_ins_index + 1 : call_ins_index + 12]
                if ins.name in set(["ADD", "MSTORE", "SUB"])
            ] == ["ADD", "SUB", "MSTORE"]:
                continue
            ...
            if (
                len(
                    [
                        succ.start
                        for succ in self._bb_at[call_ins.bb.start].succ
                        for ins in succ.ins
                        if ins.name in set(["REVERT", "INVALID"])
                    ]
                )
                != 0
            ):
            ...
            if (
                len(
                    [
                        succ.start
                        for succ in self._bb_at[call_ins.bb.start].succ
                        if [ins.name for ins in succ.ins] == ["PUSH2", "JUMP"]
                    ]
                )
                != 0
            ):
            ...
            succ_with_call_bb = [
                ...
                for succ in self._bb_at[call_ins.bb.start].succ
                for ins in succ.ins
                if ins.name in set(["CALL"])
            ]
            if ["%x" % ins.op for ins in min_succ_bb.ins][-1] in set(["fd", "fe"]) or [
                ins.name for ins in min_succ_bb.ins
            ] == ["PUSH2", "JUMP"]:
                continue
            if (
                ["%x" % ins.op for ins in min_succ_bb.ins][-1] not in set(["fd", "fe"])
                and [*["0x", "0x"], *["%x" % ins.op for ins in min_succ_bb.ins]][-3]
                not in set(["3e"])
                and [ins.name for ins in min_succ_bb.ins] != ["PUSH2", "JUMP"]
                and len(succ_with_call_bb) == 0
            ):
                instructions.append(call_ins)
            elif [*["0x"], *["%x" % ins.op for ins in min_succ_bb.ins]][-3] in set(
                ["3e"]
            ):  # RETURNDATACOPY
                call_ret_succ = [
                    succ.start
                    for succ in self._bb_at[
                        min([succ.start for succ in min_succ_bb.succ])
                    ].succ
                ]
                ret_min_succ_bb = self._bb_at[min(call_ret_succ)]
                if ["%x" % ins.op for ins in ret_min_succ_bb.ins][-1] not in set(
                    ["fd", "fe"]
                ):
                    instructions.append(call_ins)
    ...
    def find_loops(self, with_calls=False):
        ...
        for i in l:
            ...
            head = [
                ...
                and len([p.start for p in succ.pred if p.start not in i]) != 0
                and len(succ.succ) <= 2
                and (
                    "ADD" in [ins.name for ins in bb.ins]
                    or [ins.name for ins in bb.ins[-2:]] == ["PUSH2", "JUMP"]
                )
            ]
            ...
            if len(i) == 2:
                ...
                if "MLOAD" in body_ins:
                    ...
                    head_pred = [
                        ...
                        for bb in loop_bbs
                        for pred in bb.pred
                        if bb.start == head[0] and pred.start != body_start[0]
                    ]
                    head_pred1 = [pred.start for pred in self._bb_at[head_pred[0]].pred]
                    head_pred2 = [
                        pred.start for pred in self._bb_at[head_pred1[0]].pred
                    ]
                    head_pred_ins = [
                        ins.name
                        for bb in self._bb_at[head[0]].pred
                        for ins in bb.ins
                        if bb.start != body_start[0]
                        and ins.name
                        in ["ADD", "MSTORE", "JUMP", "SSTORE", "SHA3", "SLOAD"]
                    ]
                    head_pred1_ins = [
                        ins.name
                        for bb in self._bb_at[head_pred[0]].pred
                        for ins in bb.ins
                        if ins.name
                        in ["ADD", "MSTORE", "JUMP", "SSTORE", "SHA3", "SLOAD"]
                    ]
            ...
            if len(i) == 3:
                body_ins = [
                    ...
                    for bb in loop_bbs
                    for ins in bb.ins
                    if bb.start not in back_edge[0]
                    and ins.name
                    in [
                        "ADD",
                        "SUB",
                ...
                if body_ins == ["MLOAD", "MSTORE"] or body_ins == [
                    "ADD",
                    "MLOAD",
                    "ADD",
                    "MSTORE",
                ...
                if (
                    len(
                        [
                            ...
                            if bb.start == i[indx]
                            and len(bb.succ) != 0
                            and len([succ for succ in bb.succ if succ in loop_bbs])
                            == len(bb.succ)
                        ]
                    )
                    != 0
                    ...
                    len(
                        [
                            ...
                            if bb.start == i[indx]
                            and len(bb.succ) != 0
                            and len([succ for succ in bb.succ if succ in loop_bbs]) == 1
                            and len(bb.succ) == 2
                            and len(
                                [
                                    succ
                                    for succ in bb.succ
                                    if self._ins_at[succ.start].op == 254
                                ]
                            )
                            == 1
                        ]
            ...
            if not with_calls:
                for bb in loop_bbs:
                    if bb.start in loops[head[0]] and "GAS" in [
                        ins.name for ins in bb.ins
                    ]:
                        block_ins = [
                            ins.name
                            for ins in bb.ins
                            if ins.name in ["PUSH2", "PUSH3", "GAS", "GT"]
                        ]
                        if block_ins == ["PUSH3", "GAS", "GT"] or block_ins == [
                            "PUSH2",
                            "GAS",
                            "GT",
                        ]:
                            ...
                            break
                    elif bb.start not in loops[head[0]] and "GAS" in [
                        ins.name for ins in bb.ins
                    ]:
                        block_ins = [
                            ins.name
                            for ins in bb.ins
                            if ins.name in ["PUSH2", "PUSH3", "GAS", "GT", "JUMPI"]
                        ]
                        if block_ins[-5:] == [
                            "PUSH3",
                            "GAS",
                            ...
                            "PUSH2",
                            "JUMPI",
                        ] or block_ins[-5:] == ["PUSH2", "GAS", "GT", "PUSH2", "JUMPI"]:
                            loops_with_gas_sanitizers.append(head[0])
                            break
            ...
            if with_calls:
                calls_in_loop = False
                for bb in loop_bbs:
                    if bb.start not in loops[head[0]] and "CALL" in [
                        ins.name for ins in bb.ins
                    ]:
                        call_succ = [succ.start for succ in self._bb_at[bb.start].succ]
                        if len(call_succ) == 2:
                            if [
                                "%x" % ins.op for ins in self._bb_at[min(call_succ)].ins
                            ][-1] in set(["fd", "fe"]):  # revert or INVALID
                                calls_in_loop = True
                                loops_with_calls.append(head[0])
                                calls_in_loops[bb.start].append(
                                    [ins for ins in bb.ins if "CALL" == ins.name][0]
                                )
                                break
