...
def analysis(p, initial_storage=dict(),
    ...
    return TainitAnalysisBugDetails(unbounded_count, unbounded_restr_count, loop_calls_count, griefing_count, harcoded_count, asserts_count, slot_live_access_count, temp_slots_count),peak_memory_use,projectinstance.cfg.jumpcount,isworth
...
def process_file(args, initial_storage,logger,name):
    ...
    try:
        ...
        p = Project(code)
        res,mem,jcount,isworth=analysis(p, initial_storage=initial_storage) 
        ...
        logger.info(f"{name},{mem},{jcount},{isworth},{_duration}")
    # logger.info(f"{full_path.rsplit('/', 1)[-1]}, {_duration}")
    # dbname = args.file.rsplit("/", 1)[-1]
    # with open(args.file) as infile:
    #     inbuffer = infile.read().rstrip()
...
def batch_process():
    ...
    directories = [
        os.path.expanduser(
            # "/home/shuoy/repo/TaintStateMachine/datasets/popular_contracts/bytecode"
            "datasets/Ethereum/bytecode"
        ),
    ]
...
if __name__ == "__main__":
    # main()
    batch_process()
    # process_code("/home/shuoy/evaluation/eTainter/runningExample.code")